...
export class AuthController {
  ...
  public async login(@Body() login: Login): Promise<IResponse> {
    try {
      let response = await this.authService.validateLogin(login.email, login.password);
  ...
  async register(@Body() createUserDto: CreateUserDto): Promise<IResponse> {
    try {
      let newUser = new UserDto(await this.userService.createNewUser(createUserDto));
